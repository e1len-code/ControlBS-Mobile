...
class _AttendancePageEditState extends State<AttendancePageEdit> {
    ...
    _listHoursKey.asMap().forEach((index, e) {
      if (e.currentState!.controller!.text.isNotEmpty) {
        AttendanceResp? attendance =
            attendanceBloc.listAttendance.firstWhereOrNull(
        ...
        if (attendance != null) {
          ...
          attendanceBloc.add(SaveEvent(
              attendance: Attendance(
                  attnIden: 0,
                  attnline: index + 1,
                  attnDate: DateTime.parse(_dateController!.text)
                      .applyTimeOfDay(
                          hour: e.currentState!.timeOfDay!.hour,
                          minute: e.currentState!.timeOfDay!.minute),
                  attnUbic: null,
  ...
  _generarReporte(BuildContext context) async {
    attendanceBloc.add(GetReportEvent(
        attendanceReq: AttendanceReq(
            persIden: _userDropDownKey.currentState!.dropdownvalue,
            attnDtIn: DateTime.parse(_dateController!.text),
            atttnDtFn: DateTime.parse(_dateController!.text))));
    ...
    return Scaffold(
      ...
      body: BlocListener<AttendanceBloc, AttendanceState>(
        listener: (context, state) {
          ...
          } else if (state is FilteredListState) {
            for (int i = 0; i < attendanceBloc.listAttendance.length; i++) {
              AttendanceResp attendanceResp = attendanceBloc.listAttendance[i]!;
              _listHoursController[attendanceResp.attnLine - 1].text =
                  TimeOfDay.fromDateTime(attendanceResp.attnDate!)
                      .format(context);
            }
          } else if (state is GotReportState) {
            final bytes = base64Decode(state.base64Report ?? "");
        ...
        child: Padding(
          padding: const EdgeInsets.symmetric(
              vertical: vspaceM, horizontal: hspaceL),
          child: Column(
            children: <Widget>[
              Card(
                  child: Column(
